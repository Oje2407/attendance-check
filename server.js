...
// Render persistent disk path (you already mounted /var/data)
const XLSX_PATH = process.env.XLSX_PATH || path.join(__dirname, "data", "Registration.xlsx");

// Column names in your spreadsheet
...
// Passcode to protect check-in + admin download
...
function requirePasscodeHeader(req, res, next) {
  const pass = (req.header("X-Passcode") || "").toString().trim();
...
function ensureWorkbookExists() {
  // If XLSX_PATH points to /var/data/Registration.xlsx on Render,
  // and it doesn't exist yet, seed it from repo /data/Registration.xlsx
  if (fs.existsSync(XLSX_PATH)) return;

  const seedPath = path.join(__dirname, "data", "Registration.xlsx");
  if (!fs.existsSync(seedPath)) {
    throw new Error(`Seed workbook not found at ${seedPath}`);
  }

  const dir = path.dirname(XLSX_PATH);
...
app.get("/admin/download", requirePasscodeFromQuery, (req, res) => {
  try {
    ...
    return res.download(XLSX_PATH, "Registration.xlsx");
  } catch (e) {
    console.error(e);
...
// Check email + mark attendance (protected)
app.post("/api/check", requirePasscodeHeader, (req, res) => {
  try {
    ensureWorkbookExists();
    ...
    if (!fs.existsSync(XLSX_PATH)) {
      return res.status(500).json({ ok: false, message: "Registration database not found on server." });
    }
    ...
    rows[foundIndex][ATTENDANCE_TIME_COL] = new Date().toISOString();
